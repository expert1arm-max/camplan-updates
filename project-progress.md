...
- В карточке камеры добавлен явный CTA для перехода в режим редактирования, чтобы пароль и другие поля было проще менять.
- Поле пароля в карточке камеры расширено; в скрытом состоянии оно показывает маску `*`, а при `Показать пароль` раскрывает реальное значение в том же поле.
- Добавлены команды:
  - `npm run dev`
  - `npm run dev:desktop`
  - `npm run build`
