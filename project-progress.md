...
- Версия приложения поднята до `0.2.0`; локальный Windows installer `release\CCTV Manager Setup 0.2.0.exe` собран, а тег `v0.2.0` подготовлен для публикации релиза через Actions.
