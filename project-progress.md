...
- Версия приложения поднята до `0.2.7`; локальный Windows installer `release\CCTV Manager Setup 0.2.7.exe` собран, а тег `v0.2.7` отправлен в GitHub для публикации релиза через Actions.
...
- Released `0.2.7` with the latest installer-launch fix for keeping NSIS alive after closing the app.
