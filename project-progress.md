...
- Версия приложения поднята до `0.2.18`; локальный Windows installer `release\CamPlan-Installer-0.2.18.exe` собран для ручного теста update flow перед публикацией релиза.
...
- Prepared `0.2.18` locally with a temp `.cmd` update launcher plus `%TEMP%` debug/error logs; the launcher now waits 5 seconds, starts hidden during QA, and exits immediately after the installer helper starts. The GitHub release has not been published yet.
