...
- Диалог `Обновить программу` показывает loader во время проверки, затем отображает progress bar и объём скачивания, если релиз найден.
- При отсутствии обновлений диалог сообщает `Обновлено до последней версии.` и завершает проверку без бесконечного спиннера.
