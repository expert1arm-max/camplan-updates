...
- Prepared `0.2.23` locally with a hidden detached PowerShell launcher that writes `%TEMP%\CamPlanUpdateLauncher.ps1`, `%TEMP%\CamPlanUpdateDebug.log`, and `%TEMP%\CamPlanUpdateError.log`; the launcher now waits for `CamPlan.exe` to disappear by name after a short delay, starts invisibly during QA, and falls back to `shell.openPath` or detached `cmd start` if PowerShell spawn fails. A custom `build/installer.nsh` suppresses the default NSIS running-app check because the app is already closed before installation. The GitHub release has not been published yet.
