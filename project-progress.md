...
- Версия приложения поднята до `0.2.6`; локальный Windows installer `release\CCTV Manager Setup 0.2.6.exe` собран, а тег `v0.2.6` отправлен в GitHub для публикации релиза через Actions.
...
- Released `0.2.6` with the current empty-project save/export UX and the latest update/install flow fixes.
