...
- Версия приложения поднята до `0.2.20`; локальный Windows installer `release\CamPlan-Installer-0.2.20.exe` собран для ручного теста update flow перед публикацией релиза.
...
- Prepared `0.2.20` locally with a hidden detached PowerShell update launcher plus `%TEMP%` debug/error logs; the launcher now waits 2 seconds, starts invisibly during QA, and exits immediately after the installer helper starts. A custom `build/installer.nsh` suppresses the default NSIS running-app check because the app is already closed before installation. The GitHub release has not been published yet.
