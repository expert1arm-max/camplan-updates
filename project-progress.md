...
- Левая sidebar-панель переделана в lightweight explorer layout: breadcrumb сверху, scrollable tree объектов, прямой `+` для создания объекта, hover actions, inline rename и без постоянной формы редактирования.
...
- Версия приложения поднята до `0.2.26`; локальный Windows installer `release\CamPlan-Installer-0.2.26.exe` собран для ручного теста update flow перед публикацией релиза.
...
- Prepared `0.2.26` locally with a hidden detached WScript + CMD launcher that writes `%TEMP%\CamPlanUpdateLauncher.cmd`, `%TEMP%\CamPlanUpdateLauncher.vbs`, `%TEMP%\CamPlanUpdateDebug.log`, and `%TEMP%\CamPlanUpdateError.log`; the launcher now writes the scripts with `writeFileSync` before any window teardown, waits for `CamPlan.exe` to disappear by name after a short delay, and starts invisibly during QA. Startup project restore now picks the freshest persisted snapshot between IndexedDB and the `localStorage` backup instead of blindly trusting the backup copy. A custom `build/installer.nsh` suppresses the default NSIS running-app check because the app is already closed before installation; the current electron-builder schema does not accept `build.nsis.closeRunningApp`, so the runtime guard is the actual protection against self-detection regressions. The GitHub release has not been published yet.
