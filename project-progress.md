...
- Версия приложения поднята до `0.2.19`; локальный Windows installer `release\CamPlan-Installer-0.2.19.exe` собран для ручного теста update flow перед публикацией релиза.
...
- Prepared `0.2.19` locally with a temp `.cmd` update launcher plus `%TEMP%` debug/error logs; the launcher now waits 5 seconds, starts hidden during QA, and exits immediately after the installer helper starts. A custom `build/installer.nsh` suppresses the default NSIS running-app check because the app is already closed before installation. The GitHub release has not been published yet.
