...
- Версия приложения поднята до `0.2.23`; локальный Windows installer `release\CamPlan-Installer-0.2.23.exe` собран для ручного теста update flow перед публикацией релиза.
...
- Prepared `0.2.23` locally with a hidden detached PowerShell launcher that writes `%TEMP%\CamPlanUpdateLauncher.ps1`, `%TEMP%\CamPlanUpdateDebug.log`, and `%TEMP%\CamPlanUpdateError.log`; the launcher now waits for `CamPlan.exe` to disappear by name after a short delay, starts invisibly during QA, and falls back to `shell.openPath` or detached `cmd start` if PowerShell spawn fails. Startup project restore now picks the freshest persisted snapshot between IndexedDB and the `localStorage` backup instead of blindly trusting the backup copy. A custom `build/installer.nsh` suppresses the default NSIS running-app check because the app is already closed before installation. The GitHub release has not been published yet.
