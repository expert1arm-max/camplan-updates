...
- Версия приложения поднята до `0.2.1`; локальный Windows installer `release\CCTV Manager Setup 0.2.1.exe` собран, а тег `v0.2.1` подготовлен для публикации релиза через Actions.
