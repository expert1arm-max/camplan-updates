...
## Electron startup
- [ ] Запустить свежий Windows installer `release\CamPlan-Installer-0.2.36.exe`.
...
- [ ] Запустить `Помощь -> Обновить программу` и проверить, что окно показывает текущую версию приложения и версию на GitHub.
- [ ] Проверить, что приложение при старте тихо проверяет latest release на GitHub и подсвечивает кнопку `О программе` зелёным, если найдена более новая версия.
- [ ] Проверить, что кнопка `Обновить` активируется только когда версия на GitHub отличается от локальной.
...
- [ ] Verify restore logs include `restore:start`, `restore:indexeddb ...`, `restore:localstorage ...`, `restore:selected source ...`, and `restore:objects count ...`.
- [ ] Verify import/save logs include `persist:after import`, `persist:after autosave`, and `persist:after restore`.
...
- [ ] Verify the locally built `release\CamPlan-Installer-0.2.36.exe` before publishing the release.
...
- [ ] Verify `release\CamPlan-Installer-0.2.36.exe` installs and the manual update launcher still opens the installer once after the app exits.
...
- [ ] Verify `release\CamPlan-Installer-0.2.36.exe` installs without the NSIS close/retry dialog `Не удалось закрыть CamPlan`.
...
- [ ] Verify `release\CamPlan-Installer-0.2.36.exe` installs from `0.2.31` without any `РќРµ СѓРґР°Р»РѕСЃСЊ Р·Р°РєСЂС‹С‚СЊ CamPlan` dialog and without waiting for hidden input.
- [ ] Verify `release\CamPlan-Installer-0.2.36.exe` installs from `0.2.31` without any close-CamPlan dialog and without waiting for hidden input.
# Restore QA
