...
- The manual update flow now requires an explicit confirmation step after download; only then does Electron main write a real temp PowerShell launcher script, wait for the Electron PID to exit, and exit the app immediately after the launcher process is spawned. The launcher logs to `%TEMP%\CamPlanUpdateDebug.log` and `%TEMP%\CamPlanUpdateError.log`, falls back to `shell.openPath` or detached `cmd start` if PowerShell spawn fails, and keeps the installer package named separately from the app executable (`CamPlan-Installer-*` vs `CamPlan.exe`) to avoid NSIS self-detection. A custom `build/installer.nsh` override disables the default app-running process check entirely because the app is already closed before the installer starts. The helper is hidden so no visible console window appears during update launch.
