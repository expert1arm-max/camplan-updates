...
- The manual update flow now requires an explicit confirmation step after download; only then does Electron main write a temp batch launcher next to the update debug logs, launch that `.cmd` detached and hidden during QA, and exit the app immediately after the helper is spawned. The installer package is now named separately from the app executable (`CamPlan-Installer-*` vs `CamPlan.exe`) to avoid NSIS self-detection, and a custom `build/installer.nsh` override disables the default app-running process check entirely because the app is already closed before the installer starts. The launcher waits 5 seconds so NSIS starts only after the app has already shut down.
