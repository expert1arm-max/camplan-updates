...
- The manual update flow now requires an explicit confirmation step after download; only then does Electron main write a temp batch launcher next to the update debug logs, launch that `.cmd` detached and hidden during QA, and quit the app cleanly first. The launcher waits 5 seconds, and Electron main uses an `app.exit(0)` fallback after spawn so NSIS does not catch a still-running process.
