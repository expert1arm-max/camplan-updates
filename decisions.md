...
- The manual update flow now requires an explicit confirmation step after download; only then does Electron main launch a hidden detached PowerShell helper, wait for the Electron PID to exit, and exit the app immediately after the helper is spawned. The installer package is named separately from the app executable (`CamPlan-Installer-*` vs `CamPlan.exe`) to avoid NSIS self-detection, and a custom `build/installer.nsh` override disables the default app-running process check entirely because the app is already closed before the installer starts. The helper is hidden so no visible console window appears during update launch.
