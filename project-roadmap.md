...
- Проверить свежий Windows installer `release\CamPlan Setup 0.2.16.exe` на запуск, отображение стилей и update-launch логирование.
...
## Сейчас
- Проверить свежий Windows installer `release\CamPlan Setup 0.2.16.exe` на множественное выделение, раскрывающийся список в правой панели и группировку объектов.
...
- Проверить локально собранный Windows installer `release\CamPlan Setup 0.2.16.exe` на ручной сценарий обновления перед публикацией релиза.
...
- Проверить, что 5-секундная задержка и fallback `app.exit(0)` убирают prompt NSIS про закрытие CCTV Manager/CamPlan.
