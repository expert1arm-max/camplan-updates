...
- Проверить свежий Windows installer `release\CCTV Manager Setup 0.2.10.exe` на запуск и отображение стилей.
...
## Сейчас
- Проверить свежий Windows installer `release\CCTV Manager Setup 0.2.10.exe` на множественное выделение, раскрывающийся список в правой панели и группировку объектов.
...
- Verify the installer launched by the update flow is opened through a detached PowerShell `Start-Process` helper, not a child `cmd/start` process, and survives closing the app when NSIS requests it.
