...
- Проверить свежий Windows installer `release\CamPlan-Installer-0.2.20.exe` на запуск, отображение стилей и update-launch логирование.
...
## Сейчас
- Проверить свежий Windows installer `release\CamPlan-Installer-0.2.20.exe` на множественное выделение, раскрывающийся список в правой панели и группировку объектов.
...
- Проверить локально собранный Windows installer `release\CamPlan-Installer-0.2.20.exe` на ручной сценарий обновления перед публикацией релиза.
...
- Проверить, что 2-секундная задержка, immediate `app.exit(0)` и custom `build/installer.nsh` убирают prompt NSIS про закрытие CCTV Manager/CamPlan.
