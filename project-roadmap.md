...
- Проверить свежий Windows installer `release\CamPlan-Installer-0.2.29.exe` на запуск, отображение стилей и update-launch логирование.
...
- проверить restore flow: открыть JSON через любой путь, закрыть CamPlan, открыть снова и убедиться, что поднимается последний проект, а пустой старт не затирает snapshot.
...
## Сейчас
- Проверить свежий Windows installer `release\CamPlan-Installer-0.2.29.exe` на множественное выделение, раскрывающийся список в правой панели и группировку объектов.
...
- Проверить локально собранный Windows installer `release\CamPlan-Installer-0.2.29.exe` на ручной сценарий обновления перед публикацией релиза.
...
- Проверить локально собранный Windows installer `release\CamPlan-Installer-0.2.29.exe` на обычный запуск и обновление после pointer-alignment fix.
...
- Проверить локально собранный Windows installer `release\CamPlan-Installer-0.2.29.exe` на update flow без running-app close/retry prompt.
