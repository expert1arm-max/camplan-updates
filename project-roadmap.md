...
- Проверить свежий Windows installer `release\CamPlan-Installer-0.2.19.exe` на запуск, отображение стилей и update-launch логирование.
...
## Сейчас
- Проверить свежий Windows installer `release\CamPlan-Installer-0.2.19.exe` на множественное выделение, раскрывающийся список в правой панели и группировку объектов.
...
- Verify the downloaded installer keeps running after the app exits, so the NSIS prompt to close the running program does not kill the installer process.
...
- Проверить локально собранный Windows installer `release\CamPlan-Installer-0.2.19.exe` на ручной сценарий обновления перед публикацией релиза.
- Проверить, что update-install logging фиксирует путь установщика, факт существования файла, путь launcher `.cmd`, выбранный метод запуска и скрытый запуск cmd в QA.
- Проверить, что 5-секундная задержка, immediate `app.exit(0)` и custom `build/installer.nsh` убирают prompt NSIS про закрытие CCTV Manager/CamPlan.
