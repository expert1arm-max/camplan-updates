...
- Verify closing a modified project opened from a JSON file shows the native save prompt and saves back into the same file when confirmed.
...
- Проверить свежий Windows installer `release\CamPlan-Installer-0.2.32.exe` на запуск, отображение стилей и update-launch логирование.
...
- проверить restore flow: открыть JSON через любой путь, закрыть CamPlan, открыть снова и убедиться, что поднимается последний проект, а пустой старт не затирает snapshot; затем убедиться по логам, что после успешного import не было write с `objects.length = 0`.
- проверить close prompt: открыть JSON через Electron file dialog, изменить проект, закрыть окно, выбрать `Сохранить`, затем открыть CamPlan снова и убедиться, что поднимается последний проект.
...
## Сейчас
- Проверить свежий Windows installer `release\CamPlan-Installer-0.2.32.exe` на множественное выделение, раскрывающийся список в правой панели и группировку объектов.
...
- Проверить локально собранный Windows installer `release\CamPlan-Installer-0.2.32.exe` на ручной сценарий обновления перед публикацией релиза.
- Проверить, что update-install logging фиксирует путь установщика, факт существования файла, выбранный метод запуска и скрытый WScript + CMD launcher в QA.
...
- Проверить, что при рисовании стен и комнат pointer-to-SVG conversion использует точную матрицу SVG, чтобы линия/комната начинались прямо под курсором без смещения.
- Проверить локально собранный Windows installer `release\CamPlan-Installer-0.2.32.exe` на обычный запуск и обновление после pointer-alignment fix.
- Проверить, что после открытия проекта, закрытия CamPlan и повторного запуска приложение поднимает последний сохранённый проект, а `beforeunload/pagehide` flush не теряет snapshot.
- Проверить локально собранный Windows installer `release\CamPlan-Installer-0.2.32.exe` на update flow без running-app close/retry prompt.
...
- Publish release `0.2.34` with the opened-project close save prompt, then manually verify update from `0.2.33` to `0.2.34`.
