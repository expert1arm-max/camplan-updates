...
Function uninstallOldVersion
  ...
  TryInPlace:
    ...
    # We try to execute the uninstaller in place, but we do not block the installer with a retry dialog.
    ExecWait '"$uninstallerFileName" /S /KEEP_APP_DATA $0 _?=$installationDir' $R0
    ifErrors ContinueInstall

  CheckResult:
    ${if} $R0 != 0
      DetailPrint `Previous version uninstaller returned $R0; continuing install without a close/retry prompt.`
    ${endif}
