...
- Current packaged installer version is `0.2.32`.
...
- Startup restore also treats `persistedAt` and `lastSavedAt` as timestamp sources and blocks autosave until `isRestoring` clears and `hasLoadedInitialSnapshot` becomes true.
